-- =============================================================================
-- تتبّع كامل لرصيد محفظة أكثر من طبيب — حارث وسجاد (الحلو) + محمد عباس
-- شغّل كل قسم لوحده (أو كله دفعة واحدة) في Supabase → SQL Editor
-- وارسل لي كل نتائج الأقسام 1 إلى 5 كاملة — بدون أي تعديل بيانات، قراءة فقط
-- =============================================================================

-- ═══════════════════════════════════════════════════════════════════════════
-- 0) بيانات الأطباء الحالية (نسبة، نوع دفع، العيادة) — تأكد فقط
-- ═══════════════════════════════════════════════════════════════════════════
SELECT d.id AS doctor_id, d.full_name_ar AS doctor_name, d.percentage, d.payment_type,
       d.materials_share, d.clinic_id, c.name_ar AS clinic_name
FROM public.doctors d
JOIN public.clinics c ON c.id = d.clinic_id
WHERE d.full_name_ar ILIKE '%سجاد%'
   OR d.full_name_ar ILIKE '%حارث%'
...
--    لمعرفة أين يبدأ الانحراف بالضبط
-- ═══════════════════════════════════════════════════════════════════════════
WITH _docs61 AS (
  SELECT d.id AS doctor_id, d.full_name_ar AS doctor_name, d.percentage, d.payment_type,
         d.materials_share, d.clinic_id, c.name_ar AS clinic_name
...
ops AS (
  ...
  JOIN _docs61 dd ON dd.doctor_id = po.doctor_id
)
SELECT
  clinic_name,
  doctor_name,
...
GROUP BY dd.clinic_name, dd.doctor_name;
...
-- ═══════════════════════════════════════════════════════════════════════════
WITH _docs61 AS (
  SELECT d.id AS doctor_id, d.full_name_ar AS doctor_name
  FROM public.doctors d
  WHERE d.full_name_ar ILIKE '%سجاد%'
     OR d.full_name_ar ILIKE '%حارث%'
     OR d.full_name_ar ILIKE '%عباس%'
...
-- ═══════════════════════════════════════════════════════════════════════════
WITH _docs61 AS (
  SELECT d.id AS doctor_id, d.full_name_ar AS doctor_name, d.clinic_id
  FROM public.doctors d
  WHERE d.full_name_ar ILIKE '%سجاد%'
     OR d.full_name_ar ILIKE '%حارث%'
     OR d.full_name_ar ILIKE '%عباس%'
...
FROM _docs61 dd
JOIN public.clinics c ON c.id = dd.clinic_id;
