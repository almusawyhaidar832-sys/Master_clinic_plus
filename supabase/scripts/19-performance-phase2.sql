-- المرحلة 2: أداء — فهارس + تخفيف get_clinic_financial_snapshot (مسح واحد بدل 3×N)
-- شغّله في Supabase SQL Editor بعد 18-fix-accounting-consistency.sql

CREATE INDEX IF NOT EXISTS idx_transactions_doctor_type
  ON public.transactions(doctor_id, type)
  WHERE doctor_id IS NOT NULL AND amount < 0;

CREATE INDEX IF NOT EXISTS idx_operations_clinic_doctor_date
  ON public.patient_operations(clinic_id, doctor_id, operation_date);
...
-- محفظة الطبيب: JOIN واحد بدل calc لكل صف
