-- أداء الأطباء — حصة العيادة + قائمة الخاملين (بدون عمليات في الفترة)
...
BEGIN
  -- عزل صارم: نستخدم عيادة المستخدم من الملف الشخصي، لا نثق بـ p_clinic_id من العميل
  ...
  SELECT json_agg(row_to_json(d)) INTO v_top_doctors FROM (
    SELECT
      ...
      ROUND(SUM(po.clinic_share_amount)::NUMERIC, 2) AS clinic_share,
