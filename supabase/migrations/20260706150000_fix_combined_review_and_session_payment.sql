-- دفعة علاج + كشفية: is_review_statement لا يعني كشفية فقط إذا المدفوع أكبر من الكشفية
...
BEGIN
  ...
  ELSIF v_paid > 0 AND (
    ...
    OR (COALESCE(NEW.is_review_statement, FALSE) AND v_review_fee <= 0)
  ) THEN
    NEW.doctor_share_amount := 0;
    NEW.clinic_share_amount := ROUND(v_paid, 2);
...
-- إعادة حساب جلسات علاج + كشفية (بما فيها بدون review_fee_amount مسجّل)
UPDATE public.patient_operations po
SET
  review_fee_amount = sub.review_fee,
  is_review_statement = TRUE,
...
FROM (
  SELECT
    po2.id,
    CASE
      WHEN COALESCE(po2.review_fee_amount, 0) > 0 THEN COALESCE(po2.review_fee_amount, 0)
      WHEN c.review_fee_enabled AND COALESCE(c.review_fee_amount, 0) > 0
        AND COALESCE(po2.paid_amount, 0) > COALESCE(c.review_fee_amount, 0) + 0.01
        THEN COALESCE(c.review_fee_amount, 0)
      WHEN COALESCE(po2.is_review_statement, FALSE) THEN COALESCE(po2.paid_amount, 0)
      ELSE 0
    END AS review_fee,
    ROUND(
      GREATEST(
        0,
        COALESCE(po2.paid_amount, 0) - CASE
          WHEN COALESCE(po2.review_fee_amount, 0) > 0 THEN COALESCE(po2.review_fee_amount, 0)
          WHEN c.review_fee_enabled AND COALESCE(c.review_fee_amount, 0) > 0
            AND COALESCE(po2.paid_amount, 0) > COALESCE(c.review_fee_amount, 0) + 0.01
            THEN COALESCE(c.review_fee_amount, 0)
  ...
  WHERE COALESCE(po2.paid_amount, 0) > 0
    ...
    AND (
      ...
      OR (
        ...
        AND c.review_fee_enabled
        AND COALESCE(c.review_fee_amount, 0) > 0
        AND COALESCE(po2.paid_amount, 0) > COALESCE(c.review_fee_amount, 0) + 0.01
      )
    )
    AND (
      COALESCE(po2.doctor_share_amount, 0) <= 0.01
      OR COALESCE(po2.doctor_share_amount, 0) < (
        GREATEST(
          0,
          COALESCE(po2.paid_amount, 0) - COALESCE(NULLIF(po2.review_fee_amount, 0), c.review_fee_amount, 0)
        ) * (d.percentage::TEXT)::NUMERIC / 100 * 0.9
      )
    )
) sub
WHERE po.id = sub.id;
