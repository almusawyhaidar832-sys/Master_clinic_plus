-- ديون المراجعين: ذمة من زاروا خلال الفترة (وليس جمع remaining_debt لجلسات اليوم فقط)
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*)
  INTO v_revenue, v_collected, v_doctor_shares, v_clinic_shares, v_materials, v_op_count
  FROM public.patient_operations po
  WHERE po.clinic_id = p_clinic_id
    AND po.operation_date BETWEEN p_from AND p_to;

  -- ذمة المراجعين الذين زاروا خلال الفترة
  WITH visitors AS (
  ...
  SELECT COALESCE(SUM(
    CASE
      WHEN COALESCE(p.agreed_total, 0) > 0 THEN
        GREATEST(0, COALESCE(p.agreed_total, 0) - COALESCE(p.total_paid, 0))
      ELSE (
        SELECT COALESCE(MAX(GREATEST(0, po2.remaining_debt)), 0)
        FROM public.patient_operations po2
        WHERE po2.patient_id = p.id
          AND po2.clinic_id = p_clinic_id
      )
    END
  ), 0)
  INTO v_debt
  ...
  RETURN json_build_object(
    ...
    'revenue_growth',
      CASE WHEN v_prev_revenue > 0
        THEN ROUND(((v_revenue - v_prev_revenue) / v_prev_revenue) * 100, 1)
        ELSE NULL END,
    'period_from',       p_from,
    'period_to',         p_to
  );
