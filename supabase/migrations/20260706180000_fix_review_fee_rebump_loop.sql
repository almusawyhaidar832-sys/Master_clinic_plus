-- إصلاح 35,000 ← 30,000 (مضاعفة كشفية بعد migration 160000)
-- ratio 7 فقط: 35/5=7 — لا نلمس 30,000 الصحيحة (ratio 6)
...
FROM (
  ...
  WHERE COALESCE(po2.review_fee_amount, 0) > 0
    ...
    AND COALESCE(po2.paid_amount, 0) / COALESCE(po2.review_fee_amount, 1) >= 6.5
    AND COALESCE(po2.paid_amount, 0) / COALESCE(po2.review_fee_amount, 1) < 7.5
    ...
    AND ABS(
      ...
      - ROUND(
        GREATEST(
          0,
          COALESCE(po2.paid_amount, 0) - COALESCE(po2.review_fee_amount, 0) * 2
        ) * (COALESCE((d.percentage::TEXT)::NUMERIC, 50) / 100),
        2
      )
    ) > 0.02
