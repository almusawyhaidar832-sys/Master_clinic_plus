...
const DISMISS_KEY = "mcp-assistant-alerts-banner-dismissed";
...
export function AssistantAlertsSetup() {
  ...
  const registerPush = useCallback(
    (opts?: { force?: boolean; requestPermission?: boolean }) => {
      ...
      return registerAssistantWebPush(opts?.requestPermission === true, {
        forceResubscribe: opts?.force === true,
      })
        .then(async (result) => {
          const status = result.ok
            ? await fetchPushSubscriptionStatus("assistant")
            : null;
          const serverReady = (status?.subscriptionCount ?? 0) > 0;
          const ready = result.ok && serverReady;
          setPushReady(ready);
          if (!result.ok) {
            console.warn("[assistant-alerts] push registration failed:", result.reason);
  ...
  const syncPushStatus = useCallback(async (): Promise<boolean> => {
    ...
    const status = await fetchPushSubscriptionStatus("assistant");
