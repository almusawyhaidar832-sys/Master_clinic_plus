...
export function ClinicBotIntegrationPanel({ clinicId, clinicName, onMessage }: Props) {
  ...
  function handoffText(g: GeneratedResult) {
    return [
      ...
      `(لا تدخلهم داخل العقد — فقط Variables)`,
      ``,
      `--- استورد الوركفلو ---`,
      `Workflows → Import → n8n_عيادة_الامل_جاهز.json`,
      `فعّل Active = ON`,
      `لا تحذف عقدة Inquiry Assistant (مسار الاستفسارات)`,
      ``,
      `--- أرسل لي Production URL ---`,
      `افتح عقدة "Appointment Events Webhook" → Webhook URLs → Production URL`,
      `مثال: https://YOUR-N8N/webhook/appointment-events`,
      g.webhook_url ? `webhook_url الحالي: ${g.webhook_url}` : `webhook_url: (بانتظار الرابط منك)`,
      ``,
      `--- أنا أدخله بلوحة المطور → رابط Webhook → حفظ ---`,
    ].join("\n");
  }
