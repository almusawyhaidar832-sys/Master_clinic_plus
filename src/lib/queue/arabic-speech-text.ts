...
export const SPEECH_ACCOUNTANT_BILLING_TAIL =
  "، توجّه إليك للدفع — أكمل الفاتورة الآن";
...
export function splitAccountantBillingSpeech(
  patientName: string,
  gender?: PatientGender | null
): PatientCallSpeechParts {
  return {
    intro: gender === "female" ? "تم إكمال جلسة المُراجعة" : "تم إكمال جلسة المُراجع",
    patientName: formatNameForSpeech(patientName) || (gender === "female" ? "مُراجعة" : "مُراجع"),
    tail: SPEECH_ACCOUNTANT_BILLING_TAIL,
  };
}

export function buildAccountantBillingAnnouncement(
  patientName: string,
  gender?: PatientGender | null
): string {
  return joinPatientCallSpeech(splitAccountantBillingSpeech(patientName, gender));
}
