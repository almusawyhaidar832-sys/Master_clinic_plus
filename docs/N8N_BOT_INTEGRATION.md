...
## 8) سجلّ عزل التكرار العام (Idempotency) — لأي حدث خارجي

هذا endpoint **عام** وغير مرتبط بالمواعيد تحديداً — يصلح لأي حدث خارجي يصل من
أنظمتكم (وليس فقط idempotency_key الموجود أصلاً بجسم `POST /api/bot/appointments`،
والذي يبقى كما هو بدون تغيير). يفيد مثلاً عند إعادة محاولة الشبكة لإرسال حدث
ما بجانبكم وتريدون التأكد أنكم لم تعالجوه مرتين.

### التحقق قبل المعالجة

```
GET /api/bot/events/{idempotency_key}
X-Bot-Api-Key: <المفتاح>
```

- `200` + السجل إذا كان هذا المفتاح **مُعالَجاً مسبقاً** لهذه العيادة.
- `404` إذا لم يُعالَج بعد (تقدروا تكملوا المعالجة بأمان).

```json
{
  "idempotency_key": "evt_123",
  "clinic_id": "c1d2e3f4-...",
  "processed_at": "2026-07-20T09:15:00.000Z"
}
```
...
- دائماً `200` — حتى لو كان `idempotency_key` مسجَّلاً مسبقاً؛ في هذه الحالة
  يرجع السجل **الموجود** فعلاً بدون تكرار أو خطأ (`created: false`).
- `clinic_id` بالجسم اختياري ويُتجاهَل دوماً — العيادة تُحدَّد من مفتاح
  `X-Bot-Api-Key` فقط، تماماً كباقي endpoints البوت.
...
## 9) أحداث Master Clinic → N8N (Webhook صادر)
...
## 10) ما هو مؤجَّل لمرحلة قادمة (غير مُفعَّل اليوم)
...
## 11) كيف نفعّل/نعدّل عيادة عندنا
...
## 12) ملاحظات توافق مع نسخة الوركفلو "إدارة واتس اب العيادات v3"
...
- **متغيرات Notion الخاصة بكم** (`TODO_REPLACE_WITH_CLINICS_REGISTRY_DB_ID`,
  ...
  فقط، لا علاقة لنا بها — عبّوها بمعرّفات قواعدكم الحقيقية. (اختياري: بدل
  الاحتفاظ بقاعدة Notion منفصلة لعزل تكرار أحداث الـ webhook الواردة —
  عقدتا "Search Idempotency Key" و "Mark Event Processed" — يمكنكم استخدام
  `GET`/`POST /api/bot/events` عندنا (انظر القسم 8) بدل ذلك، فهو نفس الغرض
  تماماً بدون الحاجة لقاعدة Notion إضافية.)
...
### 🚫 قرار إلزامي — عدم استخدام Notion لبيانات الحجوزات إطلاقاً (من الآن فصاعداً)

تقرَّر أن تكون **Master Clinic Plus هي المصدر الوحيد** لكل ما يخص المواعيد
والحجوزات — Notion لا يُستخدم أبداً لهذا الغرض بعد اليوم، لأي عيادة جديدة أو
حالية. هذا يشمل تحديداً:

- عقدة **"Get many database pages"** و **"Get many database pages in Notion"**
  (فحص التعارض/التوافر): يجب استبدالها بالكامل باستدعاء:
  ...
  من قاعدة بيانات Master Clinic نفسها — وليس من نسخة منفصلة في Notion.

- عقدة **"Create first step"** (تسجيل جلسة/حالة "Active" في Notion) وأي
  تخزين لبيانات مريض/موعد في قاعدة "بيانات مرضى ومواعيد عيادة الأسنان":
  يجب التوقف عن تخزين أي بيانات حجز فيها. حالة المحادثة (session/memory)
  يمكن أن تبقى بأي وسيلة تقنية عندكم (مثل `Simple Memory` الموجود أصلاً بالوركفلو)
  لكن **بيانات الموعد النهائية نفسها لا تُخزَّن إلا عبر استدعاء
  `POST /api/bot/appointments`** — لا يوجد "تخزين مضاعف" بمكانين.

- عقدة **"Lookup Clinic Registry (WA)"** و **"(Webhook)"**: هذا استثناء — قاعدة
  "سجلّ العيادات" (تخزين Clinic ID / Bot API Key / رقم الواتساب) تبقى كما هي في
  Notion لأنها بيانات ربط تقنية خاصة بنظامكم، ولا علاقة لها ببيانات
  المرضى/المواعيد الطبية نفسها. القرار يخص فقط بيانات **المواعيد والحجوزات**.

- بما أن الحجز لا يحدد `doctorId` دوماً، والعيادة قد تحتوي أكثر من طبيب:
  استدعوا `GET /api/bot/clinic` أول مرة بالمحادثة لعرض قائمة الأطباء وسؤال
  المريض عن تفضيله، ثم أرسلوا `doctorId` المختار مع الحجز. إذا لم يُرسَل
...
- لعرض مواعيد مريض حالية (بدل البحث عنها في Notion): استخدموا
  `GET /api/bot/appointments?phone=07xxxxxxxxx` — يرجّع كل مواعيده القادمة
  الحقيقية من Master Clinic.
